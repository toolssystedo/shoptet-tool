...
export class ShoptetAdapter implements PlatformAdapter {
  ...
  validateFile(
    ...
    // Dynamic patterns for Shoptet columns that can have numeric suffixes (base, base2, base3, ...)
    const dynamicPatterns = [
      /^image\d*$/i,             // image, image2, ... image57
      /^categoryText\d*$/i,      // categoryText, categoryText2, ... categoryText7
      /^textProperty\d*$/i,      // textProperty, textProperty2, ...
      /^alternativeName\d*$/i,   // alternativeName, alternativeName2, ...
      /^video\d*$/i,             // video, video2, video3, ...
      /^relatedProduct\d*$/i,    // relatedProduct, relatedProduct2, ...
      /^alternativeProduct\d*$/i,// alternativeProduct, alternativeProduct2, ...
      /^gift\d*$/i,              // gift, gift2, gift3, ...
      /^surcharge\d*$/i,         // surcharge, surcharge2, ...
      /^flag\d*$/i,              // flag, flag2, flag3, ...
      /^accessories\d*$/i,       // accessories, accessories2, ...
      /^parameter\d*$/i,         // parameter, parameter2, ...
      /^variant\d*$/i,           // variant, variant2, ...
      /^availability\d*$/i,      // availability, availability2, ...
      /^priceList\d*$/i,         // priceList, priceList2, ...
    ];
    ...
    for (const header of headers) {
      ...
      const isDynamicColumn = dynamicPatterns.some((pattern) => pattern.test(header));

      if (!isAllowed && !isFilteringProperty && !isTextProperty && !isDynamicColumn) {
